...
function createProject(projectName) {
  const projectPath = path.join(ROOT_PATH, projectName);
  const BASE_PATH = path.join(projectPath, "src", "contexts");
  const BASE_PATH_SHARED = path.join(projectPath, "src", "contexts", "shared");
  const BASE_TEST_PATH = path.join(projectPath, "test");
  ...
  generateFolderContextStructure(projectName, BASE_PATH);
  copyFileConfiguration(projectName, projectPath);
  generateFolderSharedStructure(BASE_PATH_SHARED);
  generateFolderTestStructure(projectName, BASE_TEST_PATH);
}
 ...
 * @param {string} BASE_PATH - The base path for the context.
 */
function generateFolderContextStructure(contextName, BASE_PATH) {
 ...
 * @param {string} projectName - The name of the project for which configuration files are being copied.
 * @param {string} projectPath - The path of the project.
 */
function copyFileConfiguration(projectName, projectPath) {
  ...
  processAndCopyMainFiles(
    ...
    filesToCopy,
    path.join(projectPath, "src", "contexts"),
    projectName
  );
}
 ...
 * @param {string} BASE_PATH_SHARED - The base path for the shared context.
 */
function generateFolderSharedStructure(BASE_PATH_SHARED) {
  if (fs.existsSync(BASE_PATH_SHARED)) {
    console.log(`Folder structure already exists for shared context.`);
    return;
  }
  ...
  if (fs.existsSync(defaultContextPath)) {
    copyDirectory(defaultContextPath, BASE_PATH_SHARED);
  }
}
 ...
 * @param {string} BASE_TEST_PATH - The base path for the test context.
 */
function generateFolderTestStructure(contextName, BASE_TEST_PATH) {
  try {
